...
"""
TODO:
	1. Since the `Client` and `Server` are mostly the same, we should create a base class for them.
...
class Packet:
	...
	@staticmethod
	def EmptyPacket() -> 'Packet':
		ret = Packet("", header=PacketHeader(), splitter='\U000f0000')
		assert type(ret) is Packet
		return ret

	@classmethod
	def encode(cls, pkt_string:str, split_char:str='\U000f0000') -> 'Packet':
...
class Server (ANetworkManipulator):
	...
	def _handle_post_handshake(self) -> None:
		...
		self._accepted, self._accepted_addr = self._sock.accept()
